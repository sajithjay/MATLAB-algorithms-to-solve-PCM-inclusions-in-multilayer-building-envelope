...
% dt=1;
% file 
[num,txt,raw] = xlsread('material_property_nopcm.xlsx');
%% direct input
...
%% Compute new temperature: method 1
...
%                 
% %                 disp(layers(1,2).nodes)
%             if node == 1 && lay==1
%             Tnew(i) = Text;
%             i=i+1;
%             elseif node > 1 && node < layers(1,lay).nodes  
%             Tnew(i) = T(i) + ((layers(1,lay).k/(layers(1,lay).Cp*layers(1,lay).dens))*dt*(T(i+1)-2*T(i)+T(i-1))/layers(1,lay).dx^2);
% %             disp(layers(1,lay).k/(layers(1,lay).Cp*layers(1,lay).dens))
% %              disp(layers(1,lay).dx)
%             i=i+1;
...
%             end
%         end 
%% Compute new temperature: method 2         
%         for node = 1:layers(1,lay).nodes   
%                 dx=layers(1,lay).dx;
%                 k=layers(1,lay).k;
%                 cp=layers(1,lay).Cp;                
...
%             i=i+1;
%             elseif node > 1 && node < layers(1,lay).nodes  
%             T(i,n+1) = T(i,n) + ((layers(1,lay).k/(layers(1,lay).Cp*layers(1,lay).dens))*dt*(T(i+1,n)-2*T(i,n)+T(i-1,n))/layers(1,lay).dx^2);
% %             disp(layers(1,lay).k/(layers(1,lay).Cp*layers(1,lay).dens))
% %              disp(layers(1,lay).dx)
%             i=i+1;
%             elseif node == layers(1,lay).nodes && lay ~= length(layers)  
...
%                 (disp(dx))
            ...
            i=i+1;
            elseif node > 1 && node < layers(1,lay).nodes  
            T(i,n+1) = (((layers(1,lay).Cp*layers(1,lay).dx*layers(1,lay).dens/dt)*T(i,n))+((layers(1,lay).k*T(i-1,n+1))+(layers(1,lay).k*T(i+1,n+1)))/layers(1,lay).dx)/(((layers(1,lay).k+layers(1,lay).k)/layers(1,lay).dx)+layers(1,lay).Cp*layers(1,lay).dx*layers(1,lay).dens/dt);
            i=i+1;
            elseif node == layers(1,lay).nodes && lay ~= length(layers)  
            dx2=layers(1,lay+1).dx;
            k2=layers(1,lay+1).k;                
            dens2=layers(1,lay+1).dens;
            cp1_fac=layers(1,lay).Cp*layers(1,lay).dx^2*dx2*dens;
            cp2_fac=layers(1,lay+1).Cp*dx2^2*layers(1,lay).dx*dens2;                                                                                                                             
...
% % plot(time(1:30:end),sim.Qconvin(1:30:end),'-','Color','y','LineWidth',4,'MarkerSize',10) % convections
% %% 
% plot(time(1:30:end),hradint(1:30:end),'-','Color','g','LineWidth',4,'MarkerSize',10) % All above heat fluxes 
% %% 
% % plot(time(1:30:end),hrad2int(1:30:end),'-','Color','c','LineWidth',4,'MarkerSize',10) % All above heat fluxes 
% %% 
% plot(time(1:30:end),h_convi(1:30:end),'-','Color','k','LineWidth',4,'MarkerSize',10) % conductions
% %% 
% % plot(time(1:30:end),Qconvicalc(1:30:end),'-','Color',pur,'LineWidth',4,'MarkerSize',10) % conductions
...
% 
% sim.fsr= xlsread(filenamexpdata,'B2:B44641')+273.15;
% sim.flr= xlsread(filenamexpdata,'C2:C44641')+273.15;
% sim.figr= xlsread(filenamexpdata,'D2:D44641')+273.15;
% sim.fstr= xlsread(filenamexpdata,'F2:F44641')+273.15;
% sim.radhf=sim.fsr+sim.flr+sim.figr+sim.fstr;

% 
% figure
% clf
% plot(x,T(:,sim_length)-273.15);
% xlabel("x [m]")
...
% % ax = gca;
% % ax.XLim = [600 44640];
% % ax.XTick = [600 1800 3000 4200 5400 44640];
% % ax.XTickLabel = {'0','1200','2400','3600','4800','6000'};
% % title('HF')
% xlabel('Time (minutes)')
% ylabel('Temperature(\circ{C})')
% % legend={'No CLT Rockwool last presentation','No CLT rockwool','No CLT XPS,CLT Rockwool','CLT XPS','STUD with Rockwool'};
% hold on 
% % %% Setpoint 
% % plot(time,exp.TRWSP,'Color','k','LineWidth',4)
% %% BCs
% plot(sim.extbc-273.15,':','Color','k','LineWidth',4,'MarkerSize',10)
% %% Experimental 
% % plot(exp.intrf-273.15,'-','Color','k','LineWidth',4,'MarkerSize',10)
% %% MATLAB 
% % plot(Tint_sim,'-','Color',ydd,'LineWidth',4,'MarkerSize',10)
% %% Experimental 
% plot(sim.intbc-273.15,'--','Color','k','LineWidth',4,'MarkerSize',10)
% %% room 
% % plot(exp.intair1-273.15,'b--o','Color','k','LineWidth',2,'MarkerSize',6)
% hold off 
% % lgd=legend('Exterior surface', 'Experimental (ply-pou)','Experimental(pou-dry)','MATLAB (ply-pou)','MATLAB (pou-dry)','EnergyPlus Hysteresis(ply-pou)','EnergyPlus Hysteresis(pou-dry)','WUFI(ply-pou)','WUFI(pou-dry)','Interior surface');
% % lgd=legend('Radiant wall supply setpoint', 'Exterior surface', 'Experimental (ply-pou)','Experimental (pou-dry)','MATLAB (ply-pou)','MATLAB (pou-dry)','Interior surface','Chamber temperature');
% lgd=legend( '1. Exterior surface', '2. Interior surface');
